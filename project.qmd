...
By the Shannon–Nyquist theorem, the sampling frequency must exceed twice the highest signal frequency. Since our high band extends to ~8 kHz, \(f_s > 16 kHz\). We will use **48 kHz**, providing a 24 kHz Nyquist limit and headroom for filtering.

**Bit Rate:**  
At 48 kHz, 24 bits/sample, and stereo audio,  
\(R = f_s × B × C = 48{,}000 × 24 × 2 ≈ 2.3 Mbps\).  
With 32-bit I²S frames, the bit clock is ~3 MHz—easily handled by the FPGA.

**FPGA Throughput and Latency:**  
With a 50 MHz fabric clock, the FPGA has  
\(50 MHz / 48 kHz ≈ 1000\) cycles per sample—ample for three IIR filters.  
