...
execute:
  eval: true     # ensures code runs
  echo: false    # hides Python code in output (optional)
jupyter: python3
---
...
```{html}
<div id="eq_plot" style="width:100%;height:500px;"></div>
...
(function() {
    ...
    // Convert slider 0-1 to gain in dB
    function potToGainDB(pot) {
        return -maxCutDB * (1 - pot);
    }
    ...
    function lowShelfCoeffs(pot) {
        const A = dbToAmp(potToGainDB(pot));
        const w0 = 2*Math.PI*400/fs;
        const alpha = Math.sin(w0)/(2*Q);
        const cosw0 = Math.cos(w0);

        let b0 = A*((A+1)-(A-1)*cosw0 + 2*Math.sqrt(A)*alpha);
        let b1 = 2*A*((A-1)-(A+1)*cosw0);
        let b2 = A*((A+1)-(A-1)*cosw0 - 2*Math.sqrt(A)*alpha);
        let a0 = (A+1)+(A-1)*cosw0 + 2*Math.sqrt(A)*alpha;
        let a1 = -2*((A-1)+(A+1)*cosw0);
        let a2 = (A+1)+(A-1)*cosw0 - 2*Math.sqrt(A)*alpha;
    ...
    // Simple freqz function
    function freqz(b, a, N=512) {
        const w = [];
        const h = [];
        const fmin = 20;
        const fmax = fs/2;
        for(let i=0;i<N;i++){
            ...
            const z = math.complex({re: Math.cos(2*Math.PI*f/fs), im: Math.sin(2*Math.PI*f/fs)});
            const numerator = math.add(math.add(b[0], math.divide(b[1], z)), math.divide(b[2], math.pow(z,2)));
            const denominator = math.add(math.add(1, math.divide(a[0], z)), math.divide(a[1], math.pow(z,2)));
    ...
    function computeResponse(low, mid, high) {
        const bl = lowShelfCoeffs(low).b;
        const al = lowShelfCoeffs(low).a;
        const bm = midPeakingCoeffs(mid).b;
        const am = midPeakingCoeffs(mid).a;
        const bh = highShelfCoeffs(high).b;
        const ah = highShelfCoeffs(high).a;
        ...
        for(let i=0;i<N;i++){
            ...
            // For simplicity, just multiply frequency responses approx (could refine later)
            const h = 1; // placeholder for actual magnitude
            Htotal.push(h);
        }
        return {w,Htotal};
    }
    ...
    // Initial plot
    const trace = {
        x: [], y: [], type:'scatter', mode:'lines', name:'EQ Response'
    };
    const layout = {xaxis:{type:'log', title:'Frequency (Hz)'}, yaxis:{title:'Magnitude (dB)', range:[-18,3]}, title:'Interactive 3-Band EQ'};
    Plotly.newPlot('eq_plot',[trace], layout);

    // Add sliders
    const lowSlider = document.createElement('input');
    lowSlider.type='range'; lowSlider.min=0; lowSlider.max=1; lowSlider.step=0.01; lowSlider.value=1;
    const midSlider = lowSlider.cloneNode(); midSlider.value=1;
    const highSlider = lowSlider.cloneNode(); highSlider.value=1;

    const container = document.getElementById('eq_plot');
    container.appendChild(document.createElement('br')); container.appendChild(document.createTextNode('Low:')); container.appendChild(lowSlider);
    container.appendChild(document.createElement('br')); container.appendChild(document.createTextNode('Mid:')); container.appendChild(midSlider);
    container.appendChild(document.createElement('br')); container.appendChild(document.createTextNode('High:')); container.appendChild(highSlider);

    function updatePlot() {
        // TODO: Replace with proper magnitude computation
        const x = [];
        const y = [];
        const N = 512;
        const fmin = 20;
        const fmax = fs/2;
        for(let i=0;i<N;i++){
            x.push(fmin*Math.pow(fmax/fmin,i/(N-1)));
            // Placeholder: simple shape for demo
            y.push(-15*(1-lowSlider.value)*(x[i]<400 ? 1:0) -15*(1-midSlider.value)*(x[i]>=400 && x[i]<=2000 ? 1:0) -15*(1-highSlider.value)*(x[i]>2000 ? 1:0));
        }
        Plotly.react('eq_plot',[{x:x,y:y,mode:'lines',name:'EQ Response'}], layout);
    }

    lowSlider.addEventListener('input',updatePlot);
    midSlider.addEventListener('input',updatePlot);
    highSlider.addEventListener('input',updatePlot);
    updatePlot();
})();
</script>
